...
(* Le waiter demande à tour de rôle chaque philosophe s'il a faim *)
(* Quand il arrive a un philosophe faim il attend jusqu'à il est servi pour repasser aux d'autres *)
tour ==
    /\ etat[customer] = Thinking
    /\ waiterStatus = "Available"
    /\ customer' = (customer+1) % N
    /\ UNCHANGED <<etat, forksStatus, forksTaken, waiterStatus>>
...
commande(i) ==
    ...
    /\ waiterStatus = "Available"
    /\ waiterStatus' = "Taken"
    /\ UNCHANGED <<etat, forksStatus, forksTaken, customer>>
...
Fairness == \A i \in Philos :
    ...
    /\ WF_<<etat, forksStatus, forksTaken, waiterStatus, customer>> (commande(i))
    

(* Il faut une équité faible sur le changement de tour pour ne pas risquer un bégaiement infini *)
Spec ==
  /\ Init
  /\ [] [ Next ]_<<etat,forksStatus,forksTaken, waiterStatus, customer>>
  /\ Fairness
  /\ WF_<<etat, forksStatus, forksTaken, waiterStatus, customer>> (tour)
